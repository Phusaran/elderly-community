...
const ActivityDetail = () => {
  ...
  const handleJoin = async () => {
    ...
    if (!confirm('ยืนยันการจองกิจกรรมนี้')) return;

    setProcessing(true);
    try {
      await api.post(`/activities/${id}/join`);
      alert('✅ จองสำเร็จ! เตรียมตัวมาร่วมสนุกได้เลย');
  ...
  // 📌 จุดที่ 1: เพิ่มฟังก์ชัน getBadgeStyle เพื่อกำหนดสีตาม Category
  ...
  return (
    <div className="min-h-screen bg-base-200 py-10 px-4">
      <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden">
        ...
        {/* Header รูปภาพ */}
        <div className="h-64 bg-orange-600 relative flex items-center justify-center">
          <h1 className="text-3xl md:text-4xl bg-orange-600 font-bold text-white drop-shadow-md text-center px-4">{activity.title}</h1>
        </div>
          ...
          <div className="flex flex-col md:flex-row gap-8">
            ...
            <div className="flex-1 space-y-6">
              <div className="flex items-center gap-3 flex-wrap">
                <span className={`badge badge-lg ${getBadgeStyle(activity.category)} border-none`}>{activity.category}</span>
              ...
              <div>
                ...
                <p className={`leading-relaxed text-lg ${getBadgeStyle(activity.category).split(' ')[1]}`}
                >
